...
- [x] **CSS files (HTML target)** — `publisher_css` / `css`; vyasa-bg + vedabase-bg share `vysamples/styles/indic-verse.css`; intimate-note uses `vysamples/styles/prose.css`; bible is self-contained (`templates/html/theme.css` only).
